...
public class Tree {
    ...
    private void traverseInOrder(Node root) {
        ...
        traverseInOrder(root.leftChild); // 7 4 1
        System.out.println(root.value);
    ...
    public ArrayList<Integer> getNodeAtK(int distance) {
        var list = new ArrayList<Integer>();
        getNodeAtK(root, distance, list);
        return list;
    }

    private void getNodeAtK(Node root, int distance, ArrayList<Integer> list) {
        if (root == null) return;
        if (distance == 0) {
            list.add(root.value);
            return;
        }
        getNodeAtK(root.leftChild, distance-1, list);
        getNodeAtK(root.rightChild, distance-1, list);
    }

    public void traverseLevelOrder() {
        for (int i = 0; i <= height(); i++) {
            for( var value : getNodeAtK(i))
